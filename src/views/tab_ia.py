...
def consultar_ia_predict(payload):
    ...
    try:
        ...
        payload_limpo = convert_numpy(payload)

        resp = requests.post(url, json=payload_limpo, timeout=10)
...
def render_tab_ia(subestacao_obj, data_analise, dados_gd):
    ...
    with st.spinner(f"IA processando fluxo de carga para {subestacao_obj.get('nome', 'Subestação')}..."):
        ...
        if not isinstance(dict_consumo, dict): dict_consumo = {}
        ...
        if res_ia:
            if 'timeline' in res_ia and 'consumo_kwh' in res_ia:
                ...
                if not isinstance(dna_front, dict): dna_front = {}
                ...
                def pick_factor(b, f, default):
                    val_b = b if (b is not None and isinstance(b, (int, float)) and b > 0) else 0
                    val_f = f if (f is not None and isinstance(f, (int, float)) and f > 0) else 0
                    return val_b if val_b > 0 else (val_f if val_f > 0 else default)
                ...
                if consumo_res is None or consumo_com is None or consumo_ind is None:
                    ...
                    res_shape = (
                        0.3 * np.exp(-((horas - 8) ** 2) / (2 * 2.0 ** 2)) +  
                        0.5 * np.exp(-((horas - 20) ** 2) / (2 * 2.0 ** 2)) +  
                        0.2 * 0.2                                             
                    )
                    com_shape = (
                        1.0 * np.exp(-((horas - 13) ** 2) / (2 * 3.0 ** 2)) +  
                        0.05                                                   
                    )
                    ind_shape = 0.8 + 0.2 * np.sin((horas / 24.0) * 2 * np.pi)  
